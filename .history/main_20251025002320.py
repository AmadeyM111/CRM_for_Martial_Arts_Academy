...
def create_main_interface(root):
    ...
    create_students_tab(students_frame, clipboard_service, set_active_treeview)
    
    # Trainings tab
    trainings_frame = ttk.Frame(notebook)
    notebook.add(trainings_frame, text="Тренировки")
    create_trainings_tab(trainings_frame, clipboard_service, set_active_treeview)
    
    # Payments tab
    payments_frame = ttk.Frame(notebook)
    notebook.add(payments_frame, text="Платежи")
    create_payments_tab(payments_frame, clipboard_service, set_active_treeview)
    
    # Reports tab
    reports_frame = ttk.Frame(notebook)
    notebook.add(reports_frame, text="Отчеты")
    create_reports_tab(reports_frame, clipboard_service, set_active_treeview)
...
def create_students_tab(parent, clipboard_service, set_active_treeview):
    ...
    # Add sample data (unique IDs, sorted by ID)
    sample_students = [
        ...
        (4, "Анна", "Смирнова", "+7-999-456-78-90", "@smirnova", "White", "2024-04-05"),
        (5, "Михаил", "Козлов", "+7-999-567-89-01", "@kozlov", "Blue", "2024-05-12")
    ]
    ...
    original_data.clear()
    original_data.extend(sample_students)
    
    # Sort by ID and add to treeview
    sample_students.sort(key=lambda x: x[0])  # Sort by ID (first column)
    ...
    def delete_student():
        ...
        if selection:
            ...
            if tk.messagebox.askyesno("Подтверждение", message):
                ...
                original_data[:] = [item for item in original_data if item[0] not in ids_to_delete]
                
                # Delete all selected items from treeview
                for item in selection:
                    students_tree.delete(item)
...
def show_add_student_dialog(parent):
...
def show_edit_student_dialog(parent, student_data):
    ...
    def save_student():
        messagebox.showinfo("Успех", "Ученик обновлен!")
        dialog.destroy()
